...
const jpPhonemes: PhonemeDef[] = [
  ...
  {
    symbol: "N", type: "consonant", consonantType: "nasal", voiced: true, defaultDuration: 0.08,
    formants: [{ f: 250, bw: 100 }, { f: 1200, bw: 200 }, { f: 2500, bw: 250 }], antiformants: [{ f: 400, bw: 100 }]
  },
  {
  ...
  { symbol: "ts", type: "consonant", consonantType: "affricate", voiced: false, defaultDuration: 0.08, noise: { amplitude: 0.35, formantShaping: [{ f: 6000, bw: 2000 }] } },
  { symbol: "j", type: "consonant", consonantType: "affricate", voiced: true, defaultDuration: 0.07, noise: { amplitude: 0.25, formantShaping: [{ f: 3500, bw: 1500 }] } },
]
...
function romajiToPhonemes(romaji: string): string[] {
  const special: Record<string, string> = {
    "shi": "sh i", "chi": "ch i", "tsu": "ts u", "fu": "f u", "ji": "j i",
  ...
  while (i < s.length) {
    ...
    if (two === "ts" || two === "ch" || two === "sh") { result.push(two); i += 2 }
...
export const japanese: LanguageModule = {
  ...
  lyricToPhonemes(lyric: string): string[] {
    return sanitizeLyric(lyric).split(/[\s_-]+/).filter(Boolean).flatMap((p) => romajiToPhonemes(p))
  },
}
