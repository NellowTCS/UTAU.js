...
export class LFGlottalSource {
  ...
  generate(params: GlottalSourceParams, numSamples: number): Float32Array {
    ...
    const { f0, sampleRate, openQuotient, speedQuotient, tenseness, aspiration, power } = params
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      sample = sample * tilt * power + breathyNoise * aspiration * 0.1
      out[i] = sample * 0.8 + this.prevSample * 0.2
      this.prevSample = out[i]
      this.phase++
