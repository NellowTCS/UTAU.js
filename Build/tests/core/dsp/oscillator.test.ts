...
describe("LFGlottalSource", () => {
  it("produces finite samples at expected frequency", () => {
    ...
    for (let i = 0; i < n; i++) {
      samples.push(src.nextSample({ f0, sampleRate: sr, openQuotient: 0.5, speedQuotient: 0.8, tenseness: 0.6, aspiration: 0.08, power: 0.7 }))
    }
  ...
  it("handles zero f0 without crashing", () => {
    ...
    const sr = 44100
    for (let i = 0; i < 100; i++) {
      const s = src.nextSample({ f0: 0, sampleRate: sr, openQuotient: 0.5, speedQuotient: 0.8, tenseness: 0.6, aspiration: 0.08, power: 0.7 })
      expect(isFinite(s)).toBe(true)
    }
  })
